...
describe("Multilink Parser", () => {
  ...
  it("should build correct search URL", () => {
    ...
    expect(url).toBe("https://shop.multilink.us/search.php?search_query=hAP%20ac3");
  });
...
describe("model verification", () => {
  ...
  it("accepts a row that names the requested model", () => {
    const result = multilinkParser.parsePrice(MATCH_HTML, MODEL);
    expect(result).not.toBeNull();
  ...
  it("returns null when the priced row names a different product", () => {
    expect(multilinkParser.parsePrice(MISMATCH_HTML, MODEL)).toBeNull();
  });

  it("ignores verification when no model is passed", () => {
    expect(multilinkParser.parsePrice(MISMATCH_HTML)?.price).toBe(480);
  });
});
