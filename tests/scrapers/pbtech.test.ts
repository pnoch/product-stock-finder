...
describe("PBTech Parser", () => {
  ...
  it("should build correct search URL", () => {
    ...
    expect(url).toBe("https://www.pbtech.co.nz/search?sf=hAP%20ac3");
  });
...
describe("model verification", () => {
  ...
  it("accepts a row that names the requested model", () => {
    const result = pbtechParser.parsePrice(MATCH_HTML, MODEL);
    expect(result).not.toBeNull();
  ...
  it("returns null when the priced row names a different product", () => {
    expect(pbtechParser.parsePrice(MISMATCH_HTML, MODEL)).toBeNull();
  });

  it("ignores verification when no model is passed", () => {
    expect(pbtechParser.parsePrice(MISMATCH_HTML)?.price).toBe(480);
  });
});
