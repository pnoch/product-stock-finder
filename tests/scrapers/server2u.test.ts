...
describe("Server2U Parser", () => {
  ...
  it("should build correct search URL", () => {
    ...
    expect(url).toBe("https://server2u.com/shop?search=hAP%20ac3");
  });
...
describe("model verification", () => {
  ...
  it("accepts a row that names the requested model", () => {
    const result = server2uParser.parsePrice(MATCH_HTML, MODEL);
    expect(result).not.toBeNull();
  ...
  it("returns null when the priced row names a different product", () => {
    expect(server2uParser.parsePrice(MISMATCH_HTML, MODEL)).toBeNull();
  });

  it("ignores verification when no model is passed", () => {
    expect(server2uParser.parsePrice(MISMATCH_HTML)?.price).toBe(480);
  });
});
