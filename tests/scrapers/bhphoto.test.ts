...
describe("BH Photo Parser", () => {
  it("should have correct parser config", () => {
    expect(bhphotoParser.id).toBe("bhphoto-us");
    expect(bhphotoParser.baseUrl).toBe("https://bhphotovideo.com");
    expect(bhphotoParser.rateLimitMs).toBe(3000);
  });

  it("should build correct search URL", () => {
    const url = bhphotoParser.buildSearchUrl("hAP ac3");
    expect(url).toBe("https://www.bhphotovideo.com/c/search?q=hAP%20ac3");
  });

  it("should return null for 404 page fixture", () => {
    const fixturePath = path.join(FIXTURES_DIR, "bhphoto-us.html");
    expect(fs.existsSync(fixturePath)).toBe(true);

    const html = fs.readFileSync(fixturePath, "utf-8");
    const result = bhphotoParser.parsePrice(html);
    expect(result).toBeNull();
  });

  it("should return null for invalid HTML", () => {
    const result = bhphotoParser.parsePrice(
  ...
  it("should extract price, stock status, and currency from valid HTML", () => {
    const html = `<div><span class="price">$299.99</span><span class="stock-status">In Stock</span></div>`;
    const result = bhphotoParser.parsePrice(html);
...
describe("model verification", () => {
  ...
  it("accepts a row that names the requested model", () => {
    const result = bhphotoParser.parsePrice(MATCH_HTML, MODEL);
    expect(result).not.toBeNull();
  ...
  it("returns null when the priced row names a different product", () => {
    expect(bhphotoParser.parsePrice(MISMATCH_HTML, MODEL)).toBeNull();
  });
