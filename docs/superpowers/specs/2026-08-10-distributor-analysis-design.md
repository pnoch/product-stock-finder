...
Note: A distributor's `totalCost` is the sum of its **own** in-stock listing prices for the products it carries (converted to display currency). This represents the total cost if you bought each product from that distributor, so you can see which distributor is cheapest overall for your watchlist.
