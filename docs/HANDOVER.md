...
- **Phase 234 — Tauri release build:** `cargo tauri build` succeeds on Linux (webkit2gtk-4.1 present) and emits `.deb`/`.rpm`/`.AppImage` at `5.16.0`; bundle metadata (`publisher`/`copyright`/`category`/descriptions) added so the packages no longer ship `Maintainer: app` / `Description: (none)`. macOS/Windows bundles + signing still need their own build hosts.
